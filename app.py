...
def get_download_url():
    try:
        ...
        # Try method 4: Get any available format
        try:
            result = subprocess.run(
                [
                    'yt-dlp',
        ...
        try:
            ...
            logger.warning(f'Final fallback failed: {str(e)}')
        
        # All methods failed
        logger.error('All extraction methods failed')
        return jsonify({
            'success': False,
            'error': 'Could not extract download URL. The video might be unavailable or restricted.'
        }), 400
